var qrcode = function() {
  var qrcode = function(typeNumber, errorCorrectionLevel) {
    ...
    _this.createSvgTag = function(cellSize, margin, alt, title) {
      ...
      var qrSvg = '<svg version="1.1" xmlns="http://www.w3.org/2000/svg"';
  ...
  var QRUtil = function() {
    ...
    _this.getLengthInBits = function(mode, type) {
      if (1 <= type && type < 10) {
        switch(mode) { case QRMode.MODE_NUMBER: return 10; case QRMode.MODE_ALPHA_NUM: return 9; case QRMode.MODE_8BIT_BYTE: return 8; case QRMode.MODE_KANJI: return 8; default: throw 'mode:' + mode; }
      } else if (type < 27) {
        switch(mode) { case QRMode.MODE_NUMBER: return 12; case QRMode.MODE_ALPHA_NUM: return 11; case QRMode.MODE_8BIT_BYTE: return 16; case QRMode.MODE_KANJI: return 10; default: throw 'mode:' + mode; }
      } else if (type < 41) {
        switch(mode) { case QRMode.MODE_NUMBER: return 14; case QRMode.MODE_ALPHA_NUM: return 13; case QRMode.MODE_8BIT_BYTE: return 16; case QRMode.MODE_KANJI: return 12; default: throw 'mode:' + mode; }
  ...
  function qrNumber(data) {
    var _this = {};
    _this.getMode = function() { return 1; };
    _this.getLength = function() { return data.length; };
    _this.write = function(buffer) {
      var i = 0;
      while (i + 2 < data.length) { buffer.put(parseInt(data.substring(i, i + 3)), 10); i += 3; }
      if (i < data.length) {
        if (data.length - i == 1) buffer.put(parseInt(data.substring(i, i + 1)), 4);
        else if (data.length - i == 2) buffer.put(parseInt(data.substring(i, i + 2)), 7);
      }
    };
    return _this;
  }
  function qrAlphaNum(data) {
    var _this = {};
    _this.getMode = function() { return 2; };
    _this.getLength = function() { return data.length; };
    _this.write = function(buffer) {
      var i = 0;
      while (i + 1 < data.length) { buffer.put(getCode(data.charAt(i)) * 45 + getCode(data.charAt(i + 1)), 11); i += 2; }
      if (i < data.length) buffer.put(getCode(data.charAt(i)), 6);
    };
    function getCode(c) {
      if ('0' <= c && c <= '9') return c.charCodeAt(0) - 48;
      if ('A' <= c && c <= 'Z') return c.charCodeAt(0) - 55;
  ...
  function qr8BitByte(data) {
    var _bytes = [];
    for (var i = 0; i < data.length; i += 1) { var c = data.charCodeAt(i); _bytes.push(c & 0xff); }
    var _this = {};
  ...
  function createDataURL(width, height, getPixel) {
    ...
    for (var y = 0; y < height; y += 1) {
      for (var x = 0; x < width; x += 1) {
        var idx = (y * width + x) * 4;
        var p = getPixel(x, y);
        imageData.data[idx] = p ? 255 : 0;
        imageData.data[idx + 1] = p ? 255 : 0;
        imageData.data[idx + 2] = p ? 255 : 0;
